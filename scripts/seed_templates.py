...
def run_seed():
    ...
    click.echo(f"Добавление {len(sections_data)} секций...")
    for section_data in sections_data:
        section = DocumentSection(template_id=gost_template.id, **section_data)
        db.session.add(section)


    click.echo("Данные для шаблонов подготовлены.")
    return True
